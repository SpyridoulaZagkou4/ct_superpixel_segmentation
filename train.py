...
def train(cfg):
    ...
    while iterations < cfg.train_iter:
        ...
        for data in train_loader:
            ...
            values_view = metric.values() #
            value_iterator = iter(values_view) #
            first_value = next(value_iterator) #
            epoch_loss.append(first_value) #
    ...
    plt.xlabel("epochs")
    plt.ylabel("Loss ( Lrecon + λ*Lcompact)")
    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nworkers", default=2, type=int, help="number of threads for CPU parallel")
